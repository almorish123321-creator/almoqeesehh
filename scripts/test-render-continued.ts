...
async function main() {
  ...
  // Use continued:true to chain all chars together
  doc.font(fontArBold).fontSize(20).fillColor("#306db5");
  doc.text(processed[0], 50, 50, { lineBreak: false, features: [], continued: true });
  for (let i = 1; i < processed.length; i++) {
    doc.text(processed[i], { lineBreak: false, features: [], continued: i < processed.length - 1 });
  }
  doc.end();
  const chunks: Buffer[] = [];
  for await (const chunk of doc) chunks.push(Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk));
  fs.writeFileSync("/tmp/test-continued.pdf", Buffer.concat(chunks));
  console.log("Saved /tmp/test-continued.pdf");
}
